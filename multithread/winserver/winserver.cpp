...
extern C {
#endif // __cpluscplus
    void WSAAPI freeaddrinfo(addrinfo*);
    int WSAAPI getaddrinfo(const char*, const char*, const addrinfo*, addrinfo**);
#ifdef __cpluscplus
}
#endif // __cpluscplus*/
...
DWORD WINAPI threadHandler(LPVOID newPeer) {
	...
	cout << "Peer " << (int)localId << " is online" << endl;
    int state = 0;
 ...
 //       cout << recData << endl;
    ...
    cout << "Peer " << (int)localId << " is offline" << endl;
...
int main(){
	...
	for ( ts =serverInfo;ts !=NULL; ts=ts ->ai_next){
		tempSocket = socket(serverInfo ->ai_family,
	...
	cout << "Server has started up" << endl;
